...
\Head{13}{Discontinuous Galerkin (Solution)}
...
%\end{gather}
%We use the divergence theorem and substitute the boundary terms by the numerical flux $f^*$ to get \begin{gather} \label{dg} 	\sum_{n=1}^{N_p} \dtdo{\hat u_n^k}{t} \int_{D^k} \psi_n^k\psi_j^k\dif x 		-\sum_{n=1}^{N_p}a\hat u_n^k \int_{D^k} \psi_n^k \dtdo{\psi_j^k}{x}\dif x 		-\sum_{n=1}^{N_p}b\hat u_n^k \int_{D^k} \psi_n^k\psi_j^k\dif x 	=-\sqb{\psi_j^kf^*}_{x_l^k}^{x_r^k}\ . \end{gather} Equations \deq{dg} with $j=1,\ldots,N_p$, can also be written as a linear system \begin{gather} \label{dgSys} 	\hat\cM^k\dtdo{}{t}\hbu_h^k -a\rb{\hat \cS^k}^T \hbu_h^k-b\hat\cM^k\hbu_h^k 	=-\sqb{\bpsi^kf^*}_{x_l^k}^{x_r^k} \end{gather} where $\hbu_h^k=\rb{\hat u_1^k,\ldots,\hat u_{N_p}^k}^T$, and $\bpsi^k=\rb{\psi_1^k,\ldots,\psi_{N_p}^k}^T$. \\ {\bf (b)}\quad In this exercise, we suppose that $a>0$ and use the upwind flux \begin{gather} 	f_{k,k+1}^*=au_r^k\ . 	\qquad\qquad 	u_r^k=u_h^k\rb{x_r^k,\cdot}\ . \end{gather} We also write  $u_h^k\rb{x_l^k,\cdot}=u_l^k$. Multiplying \deq{dgSys} by $\hbu_h^k$, and summing over the elements provides \begin{gather} 	\dtdo{}{t}\norm{u_h}^2-b\norm{u_h}^2 	=\sum_{k=1}^K 	\bigg[ 		a\rb{u_r^k}^2 		-a\rb{u_l^k}^2 		-2u_r^k f_{k,k+1}^* 		+2u_l^k f_{k-1,k}^* 	\bigg]\ . \end{gather} Thus, we have \begin{align} 	\dtdo{}{t}\norm{u_h}^2-b\norm{u_h}^2 	&=-\sum_{k=1}^{K-1} 	\bigg[ 		-a\rb{u_r^k}^2 		+2u_r^k f_{k,k+1}^* 		+a\rb{u_l^{k+1}}^2 		-2u_l^{k+1} f_{k,k+1}^* 	\bigg]\notag\\ 	&\quad 		+a\rb{u_r^K}^2 		-2u_r^K f_{K,K+1}^* 		-a\rb{u_l^{1}}^2 		+2u_l^{1} f_{0,1}^*\ . \end{align} Since the terms \[ 	+a\rb{u_r^K}^2 		-2u_r^K f_{K,K+1}^* 		-a\rb{u_l^{1}}^2 		+2u_l^{1} f_{0,1}^* \] correspond to the boundary conditions, we will not address them here. Therefore, it is left to show that for each $k=1,\ldots,K-1$, the term \begin{gather} 	\delta^k=-a\rb{u_r^k}^2 			+2u_r^k f_{k,k+1}^* 			+a\rb{u_l^{k+1}}^2 			-2u_l^{k+1} f_{k,k+1}^* \end{gather} is nonnegative. Since clearly, \begin{gather} 	\delta^k=-a\rb{u_r^k}^2 			+2a\ u_r^k u_r^k 			+a\rb{u_l^{k+1}}^2 			-2a u_r^{k} u_l^{k+1} 		=a\rb{u_r^k-u_l^{k+1}}^2\ge0 \end{gather} the scheme is stable.
...
\begin{gather}
	...
	j=1,\ldots,N_p\ .
\end{gather}
Substituting \deq{uh_def} into the last equation, provides
\begin{gather}
...
\item
We take the dot product of \deq{dg2} and $\hbu_h^k$, and sum over $k=1,\ldots,K$ to get
\begin{gather}
	\dtdo{}{t}\norm{u_h}^2
		=\sum_{k=1}^K
			\bigg[
...
\item
By taking the dot product of \deq{DGel} and $\hat w_j^k$, and summing over $j=1,\ldots,N_p$, we get
\begin{gather}
...
$$
with $0< k\le k_{FE}$, applied to \deq{odeSys} satisfies
\begin{gather} \label{stabEst}
	\|v^{n+1}\|\le\|v^n\|
\end{gather}
in some norm $\|\cdot\|$.
Suppose $U$ is the numerical approximation of \deq{odeSys} obtained by the following 3rd-order Runga-Kutta method
\begin{align*}
