...
\begin{solutioninformation}
	...
	\begin{gather} \label{cmPDE}
		\dif{u}{t}+b\frb{x,t,u} \dif{u}{x}=c\frb{x,t,u}, \quad x \in \mathbb{R}, t \in \mathbb{R}^+
	\end{gather}
	...
	\begin{gather} \label{charODE}
		...
		x(0) = \xi.
	\end{gather}
	Under sufficient regularity conditions, one can find a unique solution of \deq{charODE} as $\hat x (\xi,t) = x(t;\xi)$ (parameterized by the initial x-intercept). Thus, along the curve $\{\hat x (\xi,t), t\}$, the PDE \deq{cmPDE} reduces to the following ODE
	\begin{gather} \label{mainODE}
		\dd{}{t}\hat u \frb{\xi,t}=c\frb{\hat x,t,\hat u} \dqquad \hat u\frb{\xi,0}=u_0\frb{\xi},
	\end{gather}
	where $\hat u(\xi,t) = u( \hat x(\xi,t),t) = u( x(t;\xi),t)$. Again, under sufficient regularity conditions we can find a unique solution for \deq{mainODE}.
	...
	\end{gather}
	The existence of a function $\hat \xi(x,t)$ satisfying \deq{xxi} is ensured if $\hat x_\xi\ne0$, due to the inverse function theorem. Thus, the solution to \deq{cmPDE},\deq{cmIC} is given by
...
\begin{solution}
	The integral form of the scalar conservation law \deq{conLaw} is given by 
	\begin{gather} \label{weakForm}
	...
	\end{gather}
	On the left hand side of \deq{AdvecIntForm} is an integral in $x$, and on the right hand side is an integral in $t$.
	
	Let $u_{0}(x)$ be any integrable function, i.e., $u_{0}(x)$ may include discontinuities. If $u(x,t)=u_0(x-at)$, then the left hand side and ride hand side of \deq{AdvecIntForm} take the form
	...
	\end{gather}
	respectively. By substituting the integration variable $y=x-at_{2}$ into the first integral of \deq{LHS}, and the integration variable $y=x-at_{1}$ into the second, we get
	\begin{gather} \label{LHS2}
		LHS=\int_{x_{1}-at_{2}}^{x_{2}-at_{2}}u_{0}\frb{y}\dif y
			-\int_{x_{1}-at_{1}}^{x_{2}-at_{1}}u_{0}\frb{y}\dif y\ .
	\end{gather}
	Similarly, by substituting $y=x_{2}-at$ into the first integral of \deq{RHS}, and $y=x_{1}-at$ in the second we get 
	\begin{gather} \label{RHS2}
		RHS=- \int_{x_{2}-at_{2}}^{x_{2}-at_{1}}u_{0}\frb{y}\dif y
			+\int_{x_{1}-at_{2}}^{x_{1}-at_{1}}u_{0}\frb{y} \dif y\ .
	\end{gather}
	Subtracting \deq{RHS2} from \deq{LHS2}, we get
	\begin{gather}
...
\begin{solution}
	The second exercise requires us to solve \deq{conLaw} with $f(u)=u^2/2$ and the initial condition
	\begin{gather}
		u_0(x)=\begin{cases}
				0 	& 	  x<-1\\
				1+x  	& -1 < x<0\\
	...
	\begin{gather}
		\dif{u}{t}+u\dif{u}{x}=0\ .
	\end{gather}
	This PDE has the form \deq{cmPDE}, with $b(x,t,u) = u$ and $c(x,t,u) = 0$.
...
\begin{solution}
	...
	\begin{gather}
		u(x,t)=\begin{cases}
			...
			\end{cases}\ ,
	\end{gather}
	satisfies \deq{WS}. We first define the functions
	\begin{gather}
		u_1(x,t)=\begin{cases}
				u_l & x<s_mt\\
				u_m & s_mt<x
	...
	\end{gather}
	Using \deq{Bsum1}, \deq{Bsum2} and \deq{Bsumm} we get the required result.
\end{solution}
...
\begin{solution}
	...
	\begin{gather} \label{charSin}
		...
		\hat x\frb{\xi,0}=\xi\ .
	\end{gather}
	The solution $\hat x$ to the ODE in \deq{charSin} is obtained fas
	\begin{gather}
