...
In Step (i), we need to solve an exact Riemann problem at each cell-interface, over a small time interval $(t_n,t_{n+1})$. Since $\wt u^n$ is a solution to the conservation law, \deq{GogAvg} yields
...
\end{gather}
since $\wt u^n$ satisfies \deq{GodIn}.
Now, let us look at the other two integrals in \deq{Godavg2}.
...
If $f'(u_l) \ge 0\ge f'(u_r)$, the entropy solution is a shock, and the intermediate state $u^*$ is determined by the sign of the shock speed \deq{RHJumpCond}.
...
\item
To show that Goduno's flux, given by \deq{GodFlx}, is monotone, we show that it is non-decreasing in its first argument and non-increasing in its second argument.
...
The exact solution to the problem with initial data \deq{InCond} is given by
\begin{gather}
	u(x,t)=\begin{cases}
		-1 & x\leq-t+1\\
